...
// The CLI plugin. It shares no code with the operator, and it reaches
// the cluster through client-go and the standard kube flags, so it
// pins the same Kubernetes libraries the operator builds against.
require (
