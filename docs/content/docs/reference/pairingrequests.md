...
| <span id="spec--ttlsecondsafterfinished"></span>`ttlSecondsAfterFinished` | integer | no | How long the request stays after it reaches Paired or Expired. The Peripheral records the request that produced it, so that record outlasts the deletion. Default: `86400`. |
