// The docs domain is its own Go module, and its only dependency is
// Hugo, pinned as a tool directive. A nested module keeps Hugo's
// dependency graph out of the operator's go.sum: the operator pins
// its Kubernetes libraries to the versions liken builds against, and
// a site generator has no business in that resolution. `go tool
// hugo` builds the pinned Hugo from source on first use and caches
// the binary, so the site needs no fetch script and no separate
// install.
//
// This is the standard Hugo build, in pure Go. The extended build
// needs cgo for its Sass compiler, and the brand theme uses plain
